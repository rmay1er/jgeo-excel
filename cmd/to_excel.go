...
Использование:
  jgeo-excel to-excel --config config.yaml
  jgeo-excel to-excel -c config.yaml`,
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		if configPath == "" {
			return fmt.Errorf("флаг --config обязателен. Используйте: to-excel --config config.yaml")
		}
...
func init() {
	...
	toExcelCmd.Flags().StringP("config", "c", "", "Путь к конфигурационному YAML файлу (обязателен)")
	toExcelCmd.MarkFlagRequired("config")
}
